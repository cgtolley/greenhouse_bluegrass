"""
song_template.py — Template for a song data file.

Format
------
SONG is a dict with two keys:
  "title"   : the song title (string)
  "sections": a list of section dicts
"""

SONG = {
    "title": "I've Been All Round This World",
    "sources": [
        # (display name, URL) — shown in the index page's Source column.
        ("rando site", "https://www.classic-country-song-lyrics.com/ivebeenallroundthisworldlyricschords.html"),
    ],
    "sections": [
        {
            "type": "solo",
            "label": "Instrumental",
            "lines": [
                ("|1    |1    |57 1 |4  1 |", None),
                ("|4    |1    |57   |C    |", None),
            ],
        },
        {
            "type": "verse",
            "label": "Verse 2",
            "lines": [
                ("1                                     ",
                 "Hang me oh hang me till I’m dead and gone"),
                ("57         1       4                 1",
                 "Hang me oh hang me till I’m dead and gone"),
                ("4                          1",
                 "One night hanging out just staying in jail too long"),
                ("   57                  1",
                 "I been all ’round this world"),
            ],
        },
        {
            "type": "verse",
            "label": "Verse 2",
            "lines": [
                ("1                                     ",
                 "Workin’ on the new railroad mud up to my knees"),
                ("57              1      4                 1",
                 "Workin’ on the new railroad mud up to my knees"),
                ("4                               1",
                 "Workin’ for ol’ John Henry he’s mighty hard to please"),
                ("   57                  1",
                 "I been all ’round this world"),
            ],
        },
        {
            "type": "verse",
            "label": "Verse 3",
            "lines": [
                ("1                                     ",
                 "Momma and papa and baby sister makes three"),
                ("57        1        4                 1",
                 "Momma and papa and baby sister makes three"),
                ("4                          1",
                 "Marchin’ me to the gallows that’ll be the end of me"),
                ("   57                  1",
                 "I been all ’round this world"),
            ],
        },
        {
            "type": "verse",
            "label": "Verse 3",
            "lines": [
                ("1                                     ",
                 "If you see a rich girl send her on down the line"),
                ("57           1         4                    1",
                 "If you see a rich girl send her on down the line"),
                ("4                           1",
                 "If you see a poor girl I’ll bet she’s a friend of mine"),
                ("   57                  1",
                 "I been all ’round this world"),
            ],
        },
        {
            "type": "verse",
            "label": "Verse 4",
            "lines": [
                ("1                                     ",
                 "Up on Blueridge Mountain there I’ll make a stand"),
                ("57                       1     4           1",
                 "Up on Blueridge Mountain there I’ll make a stand"),
                ("4                           1",
                 "Rifle on my shoulder and a six shooter in my hand"),
                ("   57                  1",
                 "I been all ’round this world"),
            ],
        },
        {
            "type": "verse",
            "label": "Verse 5",
            "lines": [
                ("1                                     ",
                 "Put the rope around my neck and hang me up so high"),
                ("57                     1        4            1",
                 "Put the rope around my neck and hung me up so high"),
                ("4                           1",
                 "Last words I heard 'em say, 'won't be long now 'fore you die, poor boy'"),
                ("   57                  1",
                 "I been all ’round this world"),
            ],
         },
    ],
}
